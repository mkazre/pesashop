...
export const ProductRelated = ({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch(`${API_URL}/api/products?category=${categoryId}&limit=8`)
      .then(r => r.json())
      .then(data => {
        if (cancelled) return;
  ...
  const displayProducts = fetched
    ? fetched.map(p => ({
        ...
        image: (() => { const img = p.featuredImage || p.images?.[0]; return img ? (img.startsWith('/uploads/') ? `${API_URL}${img}` : img) : 'https://placehold.co/200x200/e2e8f0/64748b?text=P'; })(),
        slug: p.slug,
      }))
    : products;
