...
## Phase 7 — Visual Post Designer (complete, 2026-07-14)

**This phase is fundamentally different from every one before it, and was scoped and verified accordingly.** It's a canvas editor — the deliverable is interactive UI, not backend logic — and this sandbox has no way to render or screenshot a real browser (the same Chromium install timeout that blocked Phase 2/3's visual checks applies here too, more consequentially, since a canvas editor's actual drag/resize/rotate behavior is exactly the kind of thing that can look correct in source and still be wrong on screen). **The interactive canvas itself has not been visually confirmed by me — only that it compiles, and that its non-visual logic (history stack, layer ordering) is written plainly enough to reason about directly.** MK opening `/autoposter/designer` in a real browser is the first real test of the interactive parts.

**Scope for this pass, flagged rather than silently partial**:
- Built: image, text, shape, and background layers (Spec 7.3.1/7.3.3/7.3.4/7.3.6), plus a non-visual link layer (7.3.7) for the post's destination URL. Selection/move/resize/rotate via Konva's `Transformer`. Layer panel with reorder/visibility/lock/duplicate/delete. Undo/redo (50-step history) with keyboard shortcuts (Cmd/Ctrl-Z/Shift-Z/D, Delete, arrow-key nudging). Auto-save every 10s once a design is first saved. 9 canvas presets (Spec 7.2.1). Designs Library CRUD. 6 real, editable starter templates (Spec 7.6) seeded via `autoposterSeed.js`. Bulk variant generation across presets.
- **Not built, real gaps**: video layers (7.3.2), the curated sticker/icon library (7.3.5), image filters/crop/background-removal (part of 7.3.1's tool list), snap-to-grid and alignment guides (7.4).
- **Deliberate architecture deviation from Spec 7.8, not a shortcut**: server-side rendering via "Konva-node or Puppeteer" was not built. Both options need native binaries (node-canvas's Cairo/Pango dependencies, or a Chromium download) that may not even install in a typical deployment environment — this sandbox's own Chromium install timeout earlier this session is direct evidence of that risk, not a hypothetical. Since the browser already has the fully-rendered canvas, export uses Konva's own `stage.toDataURL()` client-side, then uploads the resulting PNG through the *existing* Cloudinary media endpoint (the same one Phase 3's Composer already proved works). Same visual result, no new native-binary infrastructure to maintain or fail to install.
- **Bulk variant generation is simplified**: proportional per-axis scaling of every layer's position/size, not the spec's safe-zone-aware repositioning (re-anchoring per preset's safe zones, scaling backgrounds to fill vs. images to fit). Flagged in the code, not hidden.

**A real bug found and fixed during verification, not a simplification**: the seed script's `DESIGN_TEMPLATES` used `name` for each template's title, but `AutoposterDesign`'s actual schema field is `title` — Mongoose's strict mode correctly rejected the upsert with a clear error rather than silently writing a wrong/extra field. Fixed by renaming the 6 template entries and the seed loop's query filter; re-ran and confirmed all 6 created on first run, 0 duplicates on a second run.

**Verification performed**: 84/84 Jest tests still passing (no new tests this phase — the designer's value is in its interactive behavior, which isn't meaningfully unit-testable without a real DOM; the CRUD routes follow the same pattern already covered by every other phase's live-verification approach). Both dev servers started against the real database. Vite was confirmed to successfully pre-bundle `react-konva` (and `konva` transitively within it) with zero errors, including after forcing a dependency re-optimization. The full Designs CRUD cycle (create → list templates → get → update → delete) was run against real data, plus the seed script's idempotency was re-confirmed (6 created first run, 0 duplicates second run). All touched frontend files compile cleanly.

**Not achievable in this sandbox, by design, not skipped**: opening the Designer in a real browser and actually dragging/resizing/rotating a layer, exporting a real PNG via `toDataURL()`, and completing the Brief's literal STOP AND CONFIRM scenario (design a real new-arrival graphic and post it to a test IG account) — the latter is additionally blocked by Phase 2/5's already-known limitation (no approved platform developer apps yet).

## Phase 8 — Trend Engine: Ingestion (complete, 2026-07-15)

**Gate check done before writing any code, per the Brief's own explicit prerequisite (Section 3.9)**: queried the real database and found only 2 search events and 0 purchases in the last 14 days — the spec's own stated "highest-quality" signal is nowhere near populated. Brought this to MK directly rather than silently building around it, since this prerequisite (data volume) is different in kind from every credentials-based blocker so far (Phases 2/5) — no amount of code changes it, only real customer traffic over time does. MK chose to build the full pipeline now regardless, same call as Phases 2/5.

**New pieces**:
- `services/autoposterTrendSources.js` — one function per Spec 10.2 source. SerpAPI and X gate cleanly on missing credentials/accounts (same honest pattern as every OAuth adapter). TikTok Discover is flagged as a genuine gap, not a stub pretending to work: there's no documented public REST endpoint for TikTok trending data in the way Facebook/X expose one — only their separate Research API application (Spec 25.4) or scraping (ToS risk, fragile, not something to build blind). First-party search (SiteEvent aggregation) and first-party order velocity (24h vs 72h sell-through comparison via the real `Order` collection) are fully real and working today.
- `services/autoposterTrendScoring.js` — the Spec 10.4 composite formula (volume 0.30, velocity 0.40, source confidence 0.15, cultural event boost 0.10, cross-source validation 0.05), kept pure and DB-free for direct unit testing.
- `services/autoposterTrendIngestionRun.js` — orchestrates every source, dedupes by slug, scores, and applies brand-safety layer 1 of 3 (Spec 10.10: static blocklist matching against the terms seeded in Phase 1 — the LLM classifier and per-caption check are layers 2/3, Phase 9/10 territory). Cultural event date-matching (Spec 10.5's recurrence types) is implemented directly, including the year-boundary-spanning `annual_range` case (diaspora return season, Dec 15 → Jan 15).
- `cron/autoposterTrendIngestionCron.js` — hourly (Spec 10.3). **No Redis-lock singleton guard was added** (Spec 27.2 calls for one) — per the Phase 0 architecture decision, every cron in this codebase runs in-process in a single Node server, so the double-execution risk a lock guards against structurally doesn't exist in this deployment shape; adding one would be solving a problem this architecture doesn't have.
- `GET /api/autoposter/trends` (filterable list) and `POST /api/autoposter/trends/refresh` (manual trigger) added to `routes/autoposter.js`.

**Verification — real data producing a real, sensible result**: 101/101 Jest tests passing (17 new — scoring math, `slugify`, blocklist exact/regex matching including a deliberately-broken regex that must not crash ingestion, and cultural-event date matching for all 4 recurrence shapes). Beyond that, the real server was started and a real ingestion run triggered via the live endpoint against the real database: SerpAPI, X, and TikTok all logged their clean skip messages exactly as designed; **the Google Trends fallback hit the exact real-world failure the spec itself warns about** — Google returned an HTML page instead of JSON ("breaks periodically when Google changes markup"), and the error was caught and logged without crashing the run, proving the graceful-degradation design actually works against a real, currently-occurring failure, not just a simulated one. First-party order velocity found one genuine rising product from real order history ("Defy 4.7L Black Manual Airfryer") and scored it sensibly. A second run moments later correctly dropped its velocity to 0 (no real change in that brief window) and the score along with it — proof the velocity math responds to real elapsed time, not just a first-run default. **This trend document is real, legitimate system output, not test data — left in place, not deleted**, the same way Phase 1's seed data was meant to persist.

**Not achievable yet, by design**: SerpAPI (needs a paid key MK hasn't provisioned), X/TikTok trending (needs the same missing connected accounts as Phases 2/5), and — the real, unresolved constraint from the gate check above — genuinely useful first-party signal, which needs real customer search/purchase activity to accumulate over the coming weeks, not more code.

## Phase 9 — Trend Engine: Matching, Sampling, Safety (complete, 2026-07-15)

**New pieces**: `services/autoposterTrendProductMatcher.js` (semantic matching, Spec 10.6), `autoposterTrendSafetyClassifier.js` (brand safety layer 3 — an LLM classifies the trend *term* itself, gated on `ANTHROPIC_API_KEY` which isn't set, same honest degradation pattern as every other missing credential), `autoposterWeightedSampler.js` (the Spec 10.7 weight formula + weighted-random-without-replacement selection), `autoposterCooldownGuard.js` (region-aware hard caps, category-share caps, and recency-penalty lookups, Spec 10.8), and `autoposterTrendSamplingRun.js` (orchestrates all of the above into one run, recording a full `AutoposterDecision` audit trail per Spec 11.4). `POST /trends/sample` and `GET /decisions` added to the API.

**Two real bugs found during live verification, both fixed, neither hidden**:

1. **A genuine performance ceiling, not a bug in the matching logic**: the first version of the matcher brute-force-scanned every embedded product (7,497 and growing toward 13,700+) on every trend match. Live-tested, this didn't just run slowly — it exceeded a 3-minute timeout while sitting at ~0.6% CPU the whole time, meaning it was purely I/O-bound: transferring tens of megabytes of embedding vectors over the network per match. **MK's own instinct during this debugging session was exactly right**: narrow to relevant categories before comparing individual products. Implemented properly — not with a naive keyword filter that would defeat the point of semantic matching (e.g. "back to school" wouldn't keyword-match "Uniforms"), but by embedding each of the ~50 categories (`Category.embedding`, new additive field, same pattern as `Product.embedding`) and comparing the trend's embedding against *category* embeddings first, then only fetching full product embeddings within the top 3 matching categories. This cut the fetch from 7,497+ documents to ~40, and reduced total match time from "still running after 3+ minutes" to ~10–27 seconds. **Deeper root cause, worth knowing**: instrumented timing showed even fetching just 52 tiny category documents took 13.6 seconds, and 39 product documents took 9.6 seconds — both far slower than data volume alone explains. This points to your Atlas cluster's **Free (M0) tier** being a real, measured latency ceiling (M0 clusters are shared and throttled), not something fixable in application code. If matching speed matters going forward, the two real levers are upgrading the Atlas tier, or finally moving to Atlas Vector Search (the option flagged as "available if brute-force gets slow" back in the Phase 0 audit — it just got slow, confirmed with real numbers).
2. **A real correctness bug**: `weightedSampleWithoutReplacement` returned shallow *copies* of candidate objects (`{...c}`), which silently broke `runTrendSamplingRun.js`'s `selectedSet.has(candidate)` check — a `Set` of the returned copies, tested against the *original* objects by reference, which always returns false for objects (reference equality). Every real sampling run selected **zero** candidates despite genuine positive weights (0.38–0.67), and no existing unit test caught it, because none of them checked reference identity — they all inspected the returned objects' own properties, which were perfectly correct copies. Fixed by not copying at all (nothing in the function ever mutates a candidate's own properties, only splices the pool array), and added a dedicated regression test that checks reference identity specifically, so this exact bug class can't silently recur.

**Verification**: 116/116 Jest tests (18 new — margin/stock/platform-fit factors, the LLM classifier's graceful degradation, and Spec 26.3's explicit distribution test: 10,000 trials against a fixed weighted candidate set, asserting every candidate's selection frequency stays within 3 standard deviations of its expected probability). Beyond that, the real server was started and the full pipeline run against real data end-to-end: the real trend from Phase 8 correctly narrowed to "Air Fryers, Frying Pans, Food Processors" categories, found 14 real candidate products with sensible similarity-ranked weights, and — after the reference-equality fix — correctly selected 3 of them. The buggy run's incorrect decision records were deleted; the corrected run's real decisions were left in place as genuine system output, same as Phase 8's trend document.

**Not achievable by me, by design — needs MK's own judgment**: the Brief's literal STOP AND CONFIRM criterion for this phase is a *human* review — "manually review 20 selected candidate posts, approval rate must be > 70%." With only one real trend and 14 real candidates existing right now (thin first-party data, same known limitation from Phase 8's gate check), there isn't yet a meaningful 20-candidate sample for MK to judge, and the judgment itself (is this match actually relevant?) isn't something I can substitute for regardless of data volume — that's the whole point of the human gate.
